...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    return Ticket::new(title.clone(), description.clone(), status.clone())
        .unwrap_or_else(|e| {
            if e == "Description not provided" || e == "Description cannot be empty" || e == "Description cannot be longer than 500 bytes" {
